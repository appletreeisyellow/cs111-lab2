...
MODULE_AUTHOR("Kexin Yu, Chunchun Ye");
...
typedef struct osprd_info {
    ...
    /* added field */
    int num_read_locks;		    // Number of read locks on the device
    pid_list_t read_queue;	    // PID of read lock holders on the device
...
static int osprd_close_last(struct inode *inode, struct file *filp)
{
	...
	osp_spin_lock(&d->mutex);
        if (filp->f_flags & F_OSPRD_LOCKED){ // The file is LOCKED
            if (filp_writable){
                d->num_write_locks = 0; // num_write_locks can only be 1 or 0
                d->write_pid = -1;
            }
            else{
                d->num_read_locks--;
	...
	filp->f_flags &= ~F_OSPRD_LOCKED; // UNLOCK
                ...
                unsigned int cmd, unsigned long arg)
{
	...
	/* 1. check deadlock */
	// (These two statements are checking, 
	//  not assigning anything, 
	//  so no need to lock and unlock)
	...
	osp_spin_lock(&d->mutex);
        ...
        if(filp_writable){ // File to write
		    ...
		    d->ticket_head--;	// This process is interrupted by a signal.
					// It has to release its ticket so that other 
					// new process can use this ticket number again
            ...
            d->num_write_locks = 1;	// Number of write lock can only be 0 and 1
            ...
            /* 4. grant the read lock */
            /* critical section begins: */
            osp_spin_lock(&d->mutex);
            filp->f_flags |= F_OSPRD_LOCKED;
            d->num_read_locks++;	// record writer info
